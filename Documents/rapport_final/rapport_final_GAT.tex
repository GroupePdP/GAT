...
\bigskip
La classe SyntoxConnection permet de coordonner les éléments présents dans le package afin de concevoir une page HTML, pdp.html, permettant d'établir la communication avec Syntox. Cette page contient un formulaire où la requête est écrite et un bouton qui se charge d'envoyer le formulaire et ainsi réaliser la communication vers Syntox. L'objet de la classe SyntoxConnection reçoit en entrée la requête Syntox au préalable formatée par la méthode generateSyntox() des classes GraphConcepts et GraphNodeDefault du package linguistic. 

\bigskip
Les fichiers .txt comportent les autres éléments que le logiciel Syntox à besoin de posséder pour la génération du texte mais que nous ne devons pas générer. Parmi ces éléments nous avons la grammaire qui est stockée dans grammar.txt, le lexique dans le fichier lexicon.txt, et enfin les outils syntaxiques supplémentaires dans postEdition.txt. 

\bigskip
De la même façon, les fichiers headerPage.html et endingPage.html contiennent l'entête et le bas de la page HTML. Le premier permet la mise en place du descriptif de la page, de l'ouverture du formulaire ainsi que de la requête Syntox, tandis que le second quant à lui permet la fermeture des balises et la création du bouton permettant l'exécution du formulaire. 

\bigskip
L'utilisation de la méthode requestSyntox() de la classe SyntoxConnection permet la construction de la page HTML ainsi que le lancement automatique de cette dernière dans le navigateur par défaut de l'utilisateur. 
\bigskip
La méthode requestSyntox() fait appel à createFile(), une méthode qui va assembler chaque éléments nécessaires dans le fichier pdp.html. Nous avons dans l'ordre : 
\begin{itemize}
\item L'écriture de l'entête de la page HTML, headerPage.html. 
\item La requête Syntox, contenu dans la variable inputText. 
\item Respectivement l'ajout du contenu des fichiers grammar.txt, lexicon.txt et postEdition.txt afin de constituer le corps de la page. 
\item Le contenu du pied de page, endingPage.html pour clôturer la création de la page. 
\end{itemize}
\bigskip 
