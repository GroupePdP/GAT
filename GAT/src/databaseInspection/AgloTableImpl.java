...
public class AgloTableImpl implements AgloTable{
	...
	@Override
	public AgloTable contactRecursion(
			AgloTable cible, ArrayList<AgloTable> listAglo, int distance){
		if(distance > 0)
		{
			for(AgloTable aglo : listAglo)
			{
				...
				if(jt != null)
				{
					...
					if(aglo.equals(cible))
					{
						...
						newAglo = aglo.contactRecursion(
								cible, listAglo, distance-1);
	...
	public ArrayList<JoinTable> getJoin (
			ArrayList<AgloTable> listCible, ArrayList<AgloTable> listAglo){
		...
		for (AgloTable currentCible : listCible)
		{
			...
			for (AgloTable nextCible : listCible)
			{
				...
				{
					AgloTable newAglo = this.contactRecursion(
							nextCible, listAglo, i);
					if(newAglo != null)
					{
