	...
	public Base extractBase(DBConnection bdc){
		...
		try {
			...
			for (String nameTable : listNameTable)
			{
				...
				String requestLigne = "show columns from "
						+ nameBase + "." + nameTable;
				...
				while(resultLine.next())
				{
					...
					Column newLine = new ColumnImpl(
							lineName, lineType, lineLength, pk);
			...
			while(resultJoin.next())
			{
				...
				JoinTable newJoin = new JoinTableImpl(
						table1, tableN, key1, keyN);
