			...
			Scenario s, PanelHomeUser p)
	{
		...
		for (GraphConcepts n:scenario.getGraphList())
		{
			...
			PanelSubColumnScenario newCol = new PanelSubColumnScenario(
					thisColumnSize, currentPanel,node.getConcept(),lf,null);
		...
		this.lf = currentHome.getCurrentProject().getLinguisticFactory();
		List<Concept> tmp = this.lf.getTypeManager().getTypeTree().
				getConceptsForType(this.lf.getTypeManager().getTypeTree()
						.getRoot().getType());
		...
		this.validateButton.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				PanelSubColumnScenario newCol = new PanelSubColumnScenario(
						thisColumnSize, currentPanel,c,lf);
				newCol.setVisible(false);
				currentPanel.addColumn(newCol);
				list.add(newCol);
